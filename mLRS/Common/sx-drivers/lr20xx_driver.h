...
const tSxFlrcConfiguration Lr20xxFlrcConfiguration[] = { // 2.4 GHz, FLRC 111 Hz
    { .Bandwidth = LR20XX_FLRC_BR_650_BW_888, // that's the closest to FLRC_BR_0_650_BW_0_6 used by sx128x
      ...
      .AGCPreambleLength = LR20XX_FLRC_PREAMBLE_LENGTH_32_BITS,
      .SyncWordLength = LR20XX_FLRC_SYNCWORD_LEN_32_BITS,
      .SyncWordMatch = LR20XX_FLRC_SYNC_MATCH_1,
      .PacketType = LR20XX_FLRC_PKT_FORMAT_FIX_LEN,
      .PayloadLength = FRAME_TX_RX_LEN,
...
class Lr20xxDriverCommon : public Lr20xxDriverBase
{
  public:
    ...
    {
        SetModulationParamsFSK(config->BitRate_bps | LR20XX_FSK_BITRATE_DIRECT,
                               config->PulseShape,
                               config->Bandwidth,
                               config->Fdev_hz);
    ...
    void SetFlrcConfiguration(const tSxFlrcConfiguration* const config, uint32_t sync_word)
    {
        ...
        SetPacketParamsFLRC(config->AGCPreambleLength,
                            ...
                            LR20XX_FLRC_SYNC_TX_1,
                            LR20XX_FLRC_SYNC_MATCH_1, // config->SyncWordMatch,
                            config->PacketType,
                            config->CrcLength,
                            config->PayloadLength);

        SetSyncWordFLRC(LR20XX_FLRC_SYNCWORD_1, sync_word);
    }

    void SetFlrcConfigurationByIndex(uint8_t index, uint32_t sync_word)
    {
        if (index >= sizeof(Lr20xxFlrcConfiguration)/sizeof(Lr20xxFlrcConfiguration[0])) while(1){} // must not happen

        flrc_configuration = &(Lr20xxFlrcConfiguration[index]);
...
//??        SetRegMode(LR20XX_SIMO_USAGE_NORMAL); // Attention: requires DCDC workaround
    ...
    void _config_calc(void)
    {
        ...
        case IS_GFSK:
            ...
            break;
        default: // IS_FLRC
            flrc_configuration = &(Lr20xxFlrcConfiguration[0]);
        }
    }
    ...
    uint32_t TimeOverAir_us(void)
    {
        ...
        if (lora_configuration == nullptr && gfsk_configuration == nullptr && flrc_configuration == nullptr) {
            _config_calc(); // ensure it is set
        }

        switch (mode()) {
        case IS_LORA: return lora_configuration->TimeOverAir;
        case IS_GFSK: return gfsk_configuration->TimeOverAir;
        default: return flrc_configuration->TimeOverAir;
        }
    }
    ...
    int16_t ReceiverSensitivity_dbm(void)
    {
        ...
        switch (mode()) {
        case IS_LORA: return lora_configuration->ReceiverSensitivity;
        case IS_GFSK: return gfsk_configuration->ReceiverSensitivity;
        default: return flrc_configuration->ReceiverSensitivity;
        }
    }
  ...
  private:
    ...
    uint8_t mode(void) {
        if (gconfig->modeIsLora()) return IS_LORA;
        return (gconfig->FrequencyBand == SX_FHSS_FREQUENCY_BAND_2P4_GHZ) ? IS_FLRC : IS_GFSK;
    }
